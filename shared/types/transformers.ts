...
export const transformSharedOrderToClient = (order: SharedOrder): ClientOrder => {
  try {
    return {
      ...
      ...(order.customer_name ? { customerId: order.customer_name } : {}),
      ...
      paymentStatus: order.payment_status === 'paid' ? 'completed' : (order.payment_status as 'pending' | 'failed' | 'refunded'),
      ...(order.notes ? { specialInstructions: order.notes } : {}),
      ...(order.estimated_ready_time ? { preparationTimeMinutes: 15 } : {}),
      orderTime: validateDate(order.created_at, 'created_at'),
      ...(order.completed_at ? { completedTime: validateDate(order.completed_at, 'completed_at') } : {}),
...
export const transformClientOrderToShared = (order: ClientOrder): SharedOrder => {
  try {
    return {
      ...
      payment_status: order.paymentStatus === 'completed' ? 'paid' : (order.paymentStatus as 'pending' | 'failed' | 'refunded'),
