-- Migration: Add payment_audit_logs table
-- Issue: CRITICAL - payment.service.ts requires this table but it doesn't exist in production
-- Impact: All payment attempts fail with 500 error when audit logging is attempted
-- Source: Restored from .archive/20250201_payment_audit_logs.sql (archived Oct 20, 2025)
-- Date: 2025-10-23
...
CREATE INDEX IF NOT EXISTS idx_payment_audit_logs_restaurant_id ON payment_audit_logs(restaurant_id);
...
CREATE INDEX IF NOT EXISTS idx_payment_audit_logs_idempotency_key ON payment_audit_logs(idempotency_key);

-- Composite index for common queries
CREATE INDEX IF NOT EXISTS idx_payment_audit_logs_restaurant_created ON payment_audit_logs(restaurant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_payment_audit_logs_restaurant_status ON payment_audit_logs(restaurant_id, status);
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Migration failed: payment_audit_logs table not created';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE table_name = 'payment_audit_summary'
  ) THEN
    RAISE EXCEPTION 'Migration failed: payment_audit_summary view not created';
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_proc
    WHERE proname = 'get_payment_stats'
  ) THEN
    RAISE EXCEPTION 'Migration failed: get_payment_stats function not created';
  END IF;

  RAISE NOTICE 'Migration successful: payment_audit_logs table created';
  RAISE NOTICE 'Payment audit logging now available';
  RAISE NOTICE 'This unblocks payment processing in production';
END $$;
