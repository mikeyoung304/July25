...
## Acceptance Criteria
- [ ] All error logging sanitizes sensitive data before output
- [ ] Regex patterns detect and redact: API keys, tokens, Bearer headers, URLs with secrets
- [ ] Sanitization function has comprehensive unit tests
- [ ] Safe error fields (message, code, status) are preserved for debugging
- [ ] All external API error handling uses sanitization
- [ ] Documentation added for developers on secure logging practices
- [ ] Audit of existing logs completed to verify no credentials already leaked
- [ ] Pre-commit hook or linter rule prevents unsanitized error logging
