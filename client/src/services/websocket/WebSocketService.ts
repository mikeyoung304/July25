...
/**
 * Optimistic update tracking for rollback on connection failure
 * Allows UI to show optimistic updates and rollback if server doesn't acknowledge
 */
...
export class OptimisticWebSocketService extends WebSocketService {
  ...
  constructor(config: WebSocketConfig = {}) {
    ...
    // Listen for connection failures to trigger rollbacks
    this.on('disconnected', () => this.rollbackAllPending())
    this.on('error', () => this.rollbackAllPending())
  }

  /**
   * Register a rollback handler called when optimistic updates need to be reverted
   */
  onRollback(handler: (update: OptimisticUpdate) => void): () => void {
    this.rollbackHandlers.push(handler)
    return () => {
      const index = this.rollbackHandlers.indexOf(handler)
      if (index > -1) {
        this.rollbackHandlers.splice(index, 1)
      }
    }
  }

  /**
   * Send an optimistic update that can be rolled back on failure
   */
  sendOptimisticUpdate(data: { order_id: string; status: string; rollback_data?: unknown }): string {
    ...
    this.pendingOptimisticUpdates.set(updateId, update)

    // Send the update through WebSocket
    this.send('order:optimistic_update', {
      update_id: updateId,
      order_id: data.order_id,
      status: data.status
    })
  ...
  /**
   * Notify all rollback handlers
   */
  private notifyRollback(update: OptimisticUpdate): void {
    this.rollbackHandlers.forEach(handler => {
      try {
        handler(update)
      } catch (error) {
  ...
  /**
   * Get count of pending optimistic updates (for testing)
   */
