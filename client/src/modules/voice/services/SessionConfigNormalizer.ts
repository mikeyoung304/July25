...
export function normalizeSessionConfig(
  ...
  if (input.temperature !== undefined && input.temperature < tMin) {
    console.debug(
      `[SessionConfigNormalizer] Temperature ${input.temperature} below minimum ${tMin}, normalized to ${temperature}`
    );
  }
